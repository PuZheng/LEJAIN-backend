...
sub_nav_bar.register(url, lazy_gettext('Vendor'), enabler=
                     lambda: request.args.get('group', type=int) ==
                     const.VENDOR_GROUP)
url = speaklater.make_lazy_string(user_model_view.url_for_list,
                                  group=const.RETAILER_GROUP)
sub_nav_bar.register(url, lazy_gettext('Retailer'), enabler=
                     lambda: request.args.get('group', type=int) ==
                     const.RETAILER_GROUP)
url = speaklater.make_lazy_string(user_model_view.url_for_list,
                                  group=const.SUPER_ADMIN)
sub_nav_bar.register(url, lazy_gettext('Administrator'), enabler=
                     lambda: request.args.get('group', type=int) ==
                     const.SUPER_ADMIN)
